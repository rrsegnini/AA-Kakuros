...
def solveKakuro2(kakuro):
    ...
    if noEmptySpaces(kakuro):
        ...
        while length != 0:
            ...
            if len(threading.enumerate())<totalThreads:
            #if totalThreads<totalThreads:
                ...
                thread = threading.Thread(target=testValues,
                                args=(values, kakuro, row, column, status))
                thread.start()
                thread.join()
                ...
                if solveKakuro2(kakuro):
                    ...
                    kakuro[row][column] = BLANK_SPACE
                '''
                if testValues(values, kakuro, row, column, status):
                    return True
                else:
                    kakuro[row][column] = BLANK_SPACE
                '''
...
def testValues(values, kakuro, row, column, status):
